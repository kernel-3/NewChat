...
void TcpMsgServer::SltConnected(){
    ...
    Q_EMIT signalUserStatus(QString("用户 [%1] 上线").arg(DataBaseMgr::Instance()->GetUserName(client->GetUserId())));
...
void TcpMsgServer::SltDisConnected(){
    ...
    for (int i = 0; i < m_clients.size(); i++) {
        if (client == m_clients.at(i)){
            m_clients.remove(i);
            Q_EMIT signalUserStatus(QString("用户 [%1] 下线").arg(DataBaseMgr::Instance()->GetUserName(client->GetUserId())));
            return;
        }
    }
