...
// You may need to build the project (run Qt uic code generator) to get "ui_ServerSocket.h" resolved
...
void ClientSocket::SltDisconnected(){
    qDebug() << "disconnected";
    DataBaseMgr::Instance()->UpdateUserStatus(m_nId, OffLine);
...
void ClientSocket::ParseLogin(const QJsonValue &dataVal){
    ...
    if (dataVal.isObject()) {
        ...
        //数据库中验证
        QJsonObject jsonObj = DataBaseMgr::Instance()->CheckUserLogin(strName, strPwd);
...
void ClientSocket::ParseUserOnline(const QJsonValue &dataVal){
    ...
    if (dataVal.isArray()) {
        ...
        for (int i = 0; i < nSize; ++i) {
            ...
            int nStatus = DataBaseMgr::Instance()->GetUserLineStatus(nId);
            // 给在线的好友通报一下状态
            if (OnLine == nStatus) {
                QJsonObject jsonObj;
...
void ClientSocket::ParseLogout(const QJsonValue &dataVal){
    ...
    if (dataVal.isObject()) {
        ...
        DataBaseMgr::Instance()->UpdateUserStatus(nId, OffLine);
...
void ClientSocket::ParseUpdateUserHead(const QJsonValue &dataVal){
    ...
    if (dataVal.isObject()) {
        ...
        // 更新数据库
        DataBaseMgr::Instance()->UpdateUserHead(nId, strHead);
...
void ClientSocket::ParseReister(const QJsonValue &dataVal){
    ...
    if (dataVal.isObject()) {
        ...
        QString strName = dataObj.value("name").toString();
        QString strPwd = dataObj.value("passwd").toString();
        m_nId = DataBaseMgr::Instance()->RegisterUser(strName, strPwd);
...
void ClientSocket::ParseAddFriend(const QJsonValue &dataVal){
    ...
    if (dataVal.isObject()) {
        ...
        QJsonObject dataObj = dataVal.toObject();
        QString strName = dataObj.value("name").toString();
        QJsonObject jsonFriend = DataBaseMgr::Instance()->AddFriend(strName);
        ...
        // 给对方ID发送add请求
        QJsonObject jsonQuery = DataBaseMgr::Instance()->GetUserInfo(m_nId);
...
void ClientSocket::ParseAddGroup(const QJsonValue &dataVal){
    ...
    if (dataVal.isObject()) {
        ...
        int nId = dataObj.value("id").toInt();
        QString strName = dataObj.value("name").toString();
        QJsonObject json = DataBaseMgr::Instance()->AddGroup(nId, strName);
...
void ClientSocket::ParseCreateGroup(const QJsonValue &dataVal){
    ...
    if (dataVal.isObject()) {
        ...
        int nId = dataObj.value("id").toInt();
        QString strName = dataObj.value("name").toString();
        QJsonObject json = DataBaseMgr::Instance()->CreateGroup(nId, strName);
...
void ClientSocket::ParseGetMyFriend(const QJsonValue &dataVal){
    ...
    if (dataVal.isArray()) {
        ...
        for (int i = 0; i < nSize; ++i) {
            ...
            jsonArray.append(DataBaseMgr::Instance()->GetUserStatus(nId));
        }
    }
...
void ClientSocket::ParseGetMyGroups(const QJsonValue &dataVal){
    ...
    if (dataVal.isObject()) {
        ...
        jsonArray = DataBaseMgr::Instance()->GetGroupUsers(nId);
    }
...
void ClientSocket::ParseGroupMessages(const QByteArray &reply){
    ...
    if (!doucment.isNull() && (jsonError.error == QJsonParseError::NoError)) {
        ...
        if (doucment.isObject()) {
            ...
            // 查询该群组下面的用户，一一转发消息
            QString name = DataBaseMgr::Instance()->GetUserName(m_nId);

            // 查询该群组里面的在线好友
            QJsonArray jsonArr = DataBaseMgr::Instance()->GetGroupUsers(nGroupId);
            ...
            for (int i = 1; i < jsonArr.size(); i++) {
                ...
                if (OnLine == nStatus && m_nId != nUserId) {
                    ...
                    jsonMsg.insert("head", DataBaseMgr::Instance()->GetUserHead(m_nId));
...
void ClientFileSocket::StartTransferFile(QString fileName){
    ...
    // 要发送的文件
    fileToSend = new QFile((-2 == m_nWindowId ? MyApplication::m_strHeadPath : MyApplication::m_strRecvPath) + fileName);
...
void ClientFileSocket::SltReadyRead(){
    ...
    if (bytesReceived <= (sizeof(qint64)*2)){
                ...
                (0 != ullRecvTotalBytes))
        {
            ...
            fileToRecv->setFileName((-2 == m_nWindowId ? MyApplication::m_strHeadPath : MyApplication::m_strRecvPath) + fileReadName);
